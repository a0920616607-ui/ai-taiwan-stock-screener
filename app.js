...
async function scanPage(){
 ...
 try{
  ...
  $('#pageLabel').textContent=`第 ${page+1} 頁 / ${Math.ceil(d.total/pageSize)} 頁`;renderResults();renderWatch();
...
function renderResults(){
 ...
 $('#body').innerHTML=a.map(x=>`<tr><td><button title="${inWatch(x.code)?'管理自選股':'加入自選股'}" class="star ${inWatch(x.code)?'on':''}" onclick="chooseGroup('${x.code}')">★</button></td>
